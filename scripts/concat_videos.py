...
"""
 ...
 New Feature: Adds a pre-roll clip if the first subtitle starts late ( > 0.1s)
 to ensure the video timeline aligns perfectly with the original SRT.
"""
...
def generate_solid_color_clip(output_path, duration, params):
    """
    Generate a video clip of solid background color (#F9F6F0).
    Used for pre-roll to align timeline when the first subtitle starts late.
    """
    width, height, fps, pix_fmt = params
    # 使用模板背景色 #F9F6F0 (RGB: 249, 246, 240)
    # ffmpeg 颜色格式为 0xRRGGBB
    bg_color = "0xF9F6F0"
    ...
    try:
        ...
        print(f" [ERROR] Failed to generate solid color clip: {e.stderr.decode()}")
        return False

def generate_freeze_frame_clip(input_video, output_path, duration, params):
    """ Generate a short MP4 video of duration `duration` seconds
    by freezing the last frame of `input_video`.
...
def concat_with_freeze_frames(part_files, output_path, srt_path, split_parts):
    """ Concatenate part videos, inserting freeze-frame clips between parts
    and a pre-roll clip at the beginning if needed.
    """
    if len(part_files) < 1:
    ...
    with open(list_file, 'w', encoding='utf-8') as f:
        # ===== 新增：处理第一个Part之前的空白时段 =====
        # 如果SRT第一条字幕起始时间大于0.1秒，生成前导视频片段
        first_start_time = ranges[0][0]
        if first_start_time > 0.1:
            pre_freeze_file = os.path.join(os.path.dirname(output_path), "freeze_pre.mp4")
            print(f" Generating pre-roll clip for start time {first_start_time:.3f}s")
            
            # 生成纯色背景片段
            if generate_solid_color_clip(pre_freeze_file, first_start_time, params):
                escaped = pre_freeze_file.replace("'", "'\\''")
                f.write(f"file '{escaped}'\n")
                freeze_files.append(pre_freeze_file)
            else:
                print(" [WARN] Failed to generate pre-roll clip, timeline might be misaligned.")
        else:
            print(f" First part starts at {first_start_time:.3f}s (<=0.1s), no pre-roll needed.")
        ...
        for i, part_file in enumerate(part_files):
            ...
            if i < len(part_files) - 1:
                ...
                if effective_gap > 0.01:
                    ...
                    # print(f" Generating freeze frame for effective gap {effective_gap:.3f}s") # 调试信息可注释
...
def concat_project(project_dir, overwrite=False, srt_path=None, split_parts=None):
    ...
    if len(part_files) < 1:
        print("No video parts found.", file=sys.stderr)
        return False, None
    ...
    if srt_path and split_parts and os.path.isfile(srt_path):
        print(" Using freeze-frame gap filling method (with pre-roll check).")
